import { useFetchMovies } from '@/shared/hooks/useFetchMovie'
...
const Row:React.FC<props> = ({id,title, fetchUrl}) => {
    const {movies, loading} = useFetchMovies({fetchUrl})

    const handleScrolLeft = () => {
      const  sliderLeft = document.getElementById('slider' + id)
      if (sliderLeft) sliderLeft.scrollLeft -= 500
    }
  
    const handleScrolRight = () => {
      const  slideRight = document.getElementById('slider' + id)
