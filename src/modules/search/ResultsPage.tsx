...
import { searchMovieApi } from "./services/searchApi"
...
const ResultsPage = () => {
    ...
    useEffect(() => {
        const getResults = async () => {
            startLoading()
            if (searchQuery) {
                const data = await searchMovieApi(searchQuery, page.toString())
        ...
        getResults()
    }, [searchQuery, page, startLoading, stopLoading])
    ...
    return (
        <div className='relative w-full h-full'>
            <main className='mt-16 mx-10 md:mx-16'>
                <p className=' text-gray-500'>All results for: <span className='text-white'>{searchQuery}</span></p>
